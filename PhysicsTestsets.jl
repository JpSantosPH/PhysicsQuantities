...
### PhysicsScalar ###
    @testset "PhysicsScalar Functionalities" begin
        @test Physics(12u"m/s") == Speed(12)
        @test Speed(1) * 23 == Speed(23)
        @test Length(123) + Length(456) == Length(579)
        @test Length(12) * Frequency(3) == 36u"m/s"
    end
...
### Named units derived from SI base units ###
    @testset "Frequency Functionalities" begin
        ...
        @test Physics(12u"Hz") == Frequency(12)
    end
...
### PhysicsVector Operators ###
    @testset "PhysicsVector Operators" begin
        ...
        @test Physics(Acceleration(1,2,3) * Time(4)) isa Velocity
        @test Physics(Velocity(8, 6, 4) / Time(2)) isa Acceleration
...
### PhysicsScalar Operator ###
    @testset " PhysicsScalar Operators" begin
        ...
        @test Physics(Length(1) * Frequency(2)) isa Speed
        @test Speed(1) * 2 isa Speed
        @test Physics(Length(1) / Time(2)) isa Speed
        @test Physics(Length(1) / 2u"s") isa Speed
        @test Physics(1/2 * Mass(10) * Speed(5)^2) isa Energy
        @test dimension(Speed(12)^5) == dimension(1u"m^5/s^5") 
        @test sqrt(Length(12)) isa Quantity
        @test cbrt(Time(12)) isa Quantity
        @test Physics(inv(Time(12))) isa Frequency
        @test one(length(3)) == 1
    end
    ...
    @testset "Unitful added Functionalities" begin
        ...
        @test Physics(uconvert(u"Hz", 1/Time(1))) isa Frequency
    end