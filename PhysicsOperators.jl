...
function Base.:*(PS::PhysicsScalar, PV::PhysicsVector)
    return PhysicsVector(PS.magnitude*PV[1], PS.magnitude*PV[2], PS.magnitude*PV[3])
end
function Base.:*(PV::PhysicsVector, PS::PhysicsScalar)
    return PhysicsVector(PV[1]*PS.magnitude, PV[2]*PS.magnitude, PV[3]*PS.magnitude)
end
function Base.:/(PV::PhysicsVector, PS::PhysicsScalar)
    return PhysicsVector(PV[1]/PS.magnitude, PV[2]/PS.magnitude, PV[3]/PS.magnitude)
end
...
function Base.:+(PS₁::T, PS₂::T) where {T<:PhysicsScalar}
    return PhysicsScalar(PS₁.magnitude + PS₂.magnitude)
end
function Base.:-(PS₁::T, PS₂::T) where {T<:PhysicsScalar}
    return PhysicsScalar(PS₁.magnitude - PS₂.magnitude)
end
